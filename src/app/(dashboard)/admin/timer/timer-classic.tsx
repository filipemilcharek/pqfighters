...
export default function TimerClassic({ onToggleMode }: Props) {
  ...
  const isIdle = !running && !countingDown && !finished && remaining === 0;

  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center overflow-hidden">
      {/* Toggle mode (only when idle) */}
      {isIdle && (
        <button
          onClick={onToggleMode}
          className="absolute top-3 left-3 z-10 text-zinc-600 hover:text-zinc-400 transition-colors text-xs font-medium tracking-wide"
        >
          Modo Rounds &rarr;
        </button>
      )}
